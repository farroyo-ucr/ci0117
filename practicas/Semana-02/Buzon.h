...
class Buzon {
   public:
      Buzon();
      ~Buzon();
      int Enviar( char * mensaje, long tipo );	// Envia la tira de caracteres como un mensaje tipo
      int Enviar( void * mensaje, int len, int veces, long tipo );
      int Recibir( char * mensaje, int len, long tipo );   // len es el tamaño máximo que soporte la variable mensaje
